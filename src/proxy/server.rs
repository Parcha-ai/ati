...
"#,
        ...
        let keyring = Arc::new(Keyring::empty());
        let resolver: Arc<dyn crate::core::resolver::CredentialResolver> = Arc::new(
            crate::core::resolver::KeyringResolver::from_arc(keyring.clone()),
        );
        let state = Arc::new(ProxyState {
            registry,
            skill_registry,
...
"#
        ...
        .unwrap();
        let registry = ManifestRegistry::load(manifests_tmp.path()).expect("load manifests");
        let keyring_raw = Keyring::empty();
        let passthrough =
            crate::core::passthrough::PassthroughRouter::build(&registry, &keyring_raw)
                .expect("build passthrough router");
        let keyring = Arc::new(keyring_raw);
        let resolver: Arc<dyn crate::core::resolver::CredentialResolver> = Arc::new(
            crate::core::resolver::KeyringResolver::from_arc(keyring.clone()),
        );
        let skill_registry = SkillRegistry::load(std::path::Path::new("/nonexistent")).unwrap();
